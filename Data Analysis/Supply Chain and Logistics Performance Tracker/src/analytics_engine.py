...
class LogisticsAnalyticsEngine:
 ...
 def run_full_analysis(self):
  ...
  return {'carrier_scorecard':carrier.to_dict('records'),'top_carriers':self.carrier_scorecard.get_top_performers().to_dict('records'),'high_risk_routes':routes.to_dict('records'),'mode_analysis':self.cold_chain_analyzer.analyze_by_mode().to_dict('records'),'delay_exposure':self.cold_chain_analyzer.get_delay_exposure_proxy(),'scenario_analysis':scenarios,'savings_analysis':{'observed_sla_penalties':scenarios['observed_sla_penalties'],'total_scenario_exposure_base_35pct':float(scenarios['scenario_table'][2]['Estimated_Exposure_USD'])},'delay_trends':self.delay_analyzer.get_monthly_delay_trends().to_dict('records'),'delay_hotspots':hotspots}
